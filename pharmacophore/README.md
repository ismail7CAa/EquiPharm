...
pharmacophore/EquiPharm_Hungarian_v3/
...
`EquiPharm_Hungarian_v3` uses Euclidean embedding-space Hungarian assignment, then uses `score = -mean(|d_q - d_c|)` with Euclidean distances between matched 3D pharmacophore feature centers from RDKit/PyG metadata.

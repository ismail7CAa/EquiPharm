#!/usr/bin/env python
"""CLI for the EquiPharm Sinkhorn matching screening pipeline."""
...
try:
    from .screening import run_equipharm_sinkhorn_screening
except ImportError:
    from screening import run_equipharm_sinkhorn_screening


def parse_args():
    parser = argparse.ArgumentParser(description="Run EquiPharm Sinkhorn matching for any DUD-E target.")
    ...
    parser.add_argument("--sinkhorn-temperature", type=float)
...
def main() -> None:
    ...
    metrics = run_equipharm_sinkhorn_screening(**config)

...
import os 
import sys
sys.path.append('/project/IZZY/molecular-representation/benchmarking/') 
from equiformer_Copy3 import EquiformerQM9
...
checkpoint = torch.load("/project/IZZY/molecular-representation/benchmarking/checkpoints_Equiformer_exp/best_model.pt", map_location=device)
...
# Import data 
mol2_path = "/project/IZZY/molecular-representation/dataset/DUD-E/aces/crystal_ligand.mol2"
...
data = rdkit_mol_to_pyg_equiformer(
    ...
    y=torch.zeros((1, 19), dtype=torch.float32),  
    smiles=smiles,
    name="DUD-E_aces_crystal_ligand",
    idx=0
)
# print(data)
...
def main():
    ...
    checkpoint = torch.load(
        "/project/IZZY/molecular-representation/benchmarking/checkpoints_Equiformer_exp/best_model.pt",
        map_location=device
    )
    # print(checkpoint.keys())
    ...
    # Actives and Decoys
    folder_active = "/project/IZZY/molecular-representation/dataset/DUD-E/aces/actives_sdf"
    folder_decoy  = "/project/IZZY/molecular-representation/dataset/DUD-E/aces/decoys_sdf"
    ...
    plt.tight_layout()
    plt.savefig("cosine_similarity_boxplot.png", dpi=100, bbox_inches="tight")
    plt.show()

...
OUT_ROOT="${OUT_ROOT:-pharmacophore/results/PharmacoMatch_DEKOIS2.0_subset_${RUN_LABEL}}"
LOG_ROOT="${LOG_ROOT:-pharmacophore/results/PharmacoMatch_DEKOIS2.0_subset_${RUN_LABEL}_logs}"
WORK_ROOT="${WORK_ROOT:-pharmacophore/work/PharmacoMatch_DEKOIS2.0_subset_${RUN_LABEL}}"
...
if [ "$#" -eq 0 ]; then
    ...
    echo 'Optional: SEEDS="1 2 3" N_ACTIVES=40 N_DECOYS=500 BATCH_SIZE=34 bash scripts/run_pharmacomatch_dekois2_subset_targets.sh ace'
    exit 1
fi

for REQUIRED in "$HELPER" "$AGGREGATOR"; do
    if [ ! -f "$REQUIRED" ]; then echo "[FAIL] Missing helper: $REQUIRED"; exit 1; fi
done
...
for REQUESTED_TARGET in "$@"; do
    ...
    for SEED in $SEEDS; do
        ...
        PREPARED_TARGET="$PHARM_ROOT/data/DEKOIS2.0/$RUN_NAME"
        TARGET_OUT="$OUT_ROOT/seed_$SEED/$TARGET"
        TARGET_LOG="$LOG_ROOT/seed_$SEED/$TARGET"

        rm -rf "$SUBSET_TARGET" "$PREPARED_TARGET" "$TARGET_OUT" "$TARGET_LOG"
        mkdir -p "$SUBSET_TARGET/actives_sdf" "$SUBSET_TARGET/decoys_sdf" "$TARGET_OUT" "$TARGET_LOG"
        cp "$SOURCE_TARGET/crystal_ligand.mol2" "$SUBSET_TARGET/crystal_ligand.mol2"

        echo "[$TARGET seed $SEED] Selecting $N_ACTIVES actives and $N_DECOYS decoys"
        python "$HELPER" --input "$SOURCE_TARGET/actives_sdf" \
            --output "$SUBSET_TARGET/actives_sdf/actives.sdf" --count "$N_ACTIVES" \
            --seed "$SEED" --target "$TARGET" --class-label active \
        ...
        python "$HELPER" --input "$SOURCE_TARGET/decoys_sdf" \
            --output "$SUBSET_TARGET/decoys_sdf/decoys.sdf" --count "$N_DECOYS" \
            --seed "$SEED" --target "$TARGET" --class-label inactive \
        ...
        python -m pharmacophore.PharmacoMatch.cli \
            ...
            --force-prepare --accelerator cuda --devices 1 --batch-size "$BATCH_SIZE" \

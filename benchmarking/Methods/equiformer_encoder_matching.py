...
class EquiformerQM9(nn.Module):
    ...
    def masked_mean_pool(self, atom_embeddings, mask):
        ...
        mask: (N,) boolean
        """
        mask_f = mask.float().unsqueeze(-1)  # (N, 1)  *Convert boolean mask to float*---> allows padded atoms to be zeroed out
        denom = mask_f.sum(dim=0).clamp_min(1.0)  # (1,) *Count how many valid atoms there are.* , clamp_min to avoid division by zero
        pooled = (atom_embeddings * mask_f).sum(dim=0) / denom # masked averaging : - invalid padded atoms becom zero, - sum valid atomo embeddings, -divide by numbeerof V.A
        return pooled
    ...
    def _iter_valid_pharmacophore_features(self, pharmacophore_features, num_atoms, mask=None):
        for feat in pharmacophore_features:
            if isinstance(feat, list):
                yield from self._iter_valid_pharmacophore_features(feat, num_atoms, mask=mask)
                continue
    ...
    def pharmacophore_feature_embeddings(self, atom_embeddings, pharmacophore_features, mask=None, coords=None):
        ...
        query-feature by candidate-feature similarity matrix.
        """
        feat_embs = []
        metadata = []
        num_atoms = atom_embeddings.size(0)

        for feat, valid_ids in self._iter_valid_pharmacophore_features(
        ...
        return torch.stack(feat_embs, dim=0), metadata
    ...
    def encode_pharmacophore_features(self, data):
        ...
        for b in range(x.size(0)):
            ...
            feature_embeddings, feature_metadata = self.pharmacophore_feature_embeddings(
                atom_embeddings=atom_emb_b,
            ...
            outputs.append(
                {
                    ...
                    "global_embedding": self.masked_mean_pool(atom_emb_b, mask_b),
                }
            )

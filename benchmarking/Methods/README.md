...
Both Equiformer entry points can train all 19 QM9 targets (the default), a named
target group, or one target. Use a separate output directory for each experiment
so checkpoints from different output heads cannot be mixed.
...
python benchmarking/Methods/equiformer_adj.py \
  --target-preset all --output-dir runs/EquiformerAdj/all --device cuda
python benchmarking/Methods/equiformer_pt_cloud.py \
  --target-preset all --output-dir runs/EquiformerPointCloud/all --device cuda

# Electronic group
python benchmarking/Methods/equiformer_adj.py \
  --target-preset electronic --output-dir runs/EquiformerAdj/electronic --device cuda
python benchmarking/Methods/equiformer_pt_cloud.py \
  --target-preset electronic --output-dir runs/EquiformerPointCloud/electronic --device cuda

# Geometry group
python benchmarking/Methods/equiformer_adj.py \
  --target-preset geometry --output-dir runs/EquiformerAdj/geometry --device cuda
python benchmarking/Methods/equiformer_pt_cloud.py \
  --target-preset geometry --output-dir runs/EquiformerPointCloud/geometry --device cuda
...
```bash
# All 19 targets, seeds 1/2/3 and a different split for each seed
python benchmarking/Methods/equiformer_official_benchmark.py \
  --target-preset all --seeds 1 2 3 \
  --output-dir runs/EquiformerOfficialBenchmark/all --device cuda

# Six electronic targets with explicit training and split seeds
python benchmarking/Methods/equiformer_official_benchmark.py \
  --target-preset electronic --seeds 4 5 6 \
...
By default, repeated `--seeds` vary both training and split seeds. Add
`--fixed-split --split-seed N` to vary initialization/training randomness while
holding the dataset split constant.
